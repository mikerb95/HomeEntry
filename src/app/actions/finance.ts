...
export async function updateMoraConfig(
  ...
  // Effective ceiling: the certified rate fetched by /api/cron/usura when
  // available, never above the conservative constant fallback.
  const capPct = await getEffectiveMoraCapPct();
  if (moraRatePct > capPct)
    return {
      ok: false,
      error: `La tasa de mora no puede superar el tope legal vigente de ${(capPct / 100).toFixed(2)}% mensual (1.5x el interés bancario corriente certificado por la Superfinanciera)`,
    };
